...
import { User, Mail, Phone, MapPin, ShieldAlert, Check, Eye, EyeOff, Save, ArrowLeft, Loader2, Send, Unlock } from 'lucide-react';
...
const EditProfile = () => {
  ...
  const isRestrictedRole = currentRole === 'worker' || currentRole === 'dept' || currentRole === 'official';
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        if (isRestrictedRole) {
          const statusRes = await fetch("http://localhost:8000/api/auth/edit-request/status", {
            headers: { "Authorization": `Bearer ${token}` }
          });
          if (statusRes.ok) {
  ...
  const handleSendRequest = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("http://localhost:8000/api/auth/request-edit", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  if (isFetching) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4">
        ...
        <p className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Decrypting Identity Record...</p>
      </div>
    );
  }
      ...
      <div className="vault-card p-6 md:p-8 border border-zinc-800/80 shadow-[0_0_40px_rgba(0,0,0,0.5)] rounded-xl bg-zinc-950/80 backdrop-blur-xl">
        ...
        <div className="flex flex-col items-center mb-10">
          <div className="w-24 h-24 rounded-full bg-zinc-900 border-2 border-emerald-500/50 flex items-center justify-center mb-4 shadow-[0_0_20px_rgba(16,185,129,0.15)] relative group overflow-hidden">
            <span className="text-3xl font-bold text-emerald-400">{formData.fullName.charAt(0) || 'U'}</span>
            {!isFormLocked && (
              <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                <span className="text-[10px] font-mono text-white uppercase tracking-widest">Edit</span>
              </div>
            )}
          </div>
          <h3 className="text-lg font-bold text-zinc-200 uppercase tracking-wide">{formData.fullName || 'Authorized User'}</h3>
          ...
          <div className="mb-8 flex flex-col gap-4">
            ...
            {requestStatus === "Approved" ? (
              <div className="p-4 bg-emerald-950/30 border-l-4 border-emerald-500 rounded-r-lg flex items-start gap-3 shadow-lg">
                <Unlock className="text-emerald-500 shrink-0 mt-0.5" size={18} />
                <div className="flex-1">
                  <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-widest mb-1">Editing Authorized</h4>
                  <p className="text-[10px] font-mono text-zinc-300 uppercase tracking-wider leading-relaxed">
                    Command Center has temporarily unlocked your profile. You may update your credentials. This permission will be securely revoked immediately after saving.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-amber-950/30 border-l-4 border-amber-500 rounded-r-lg flex items-start gap-3 shadow-lg">
                ...
                <div className="flex-1">
                  <h4 className="text-xs font-bold text-amber-400 uppercase tracking-widest mb-1">Editing Restricted</h4>
                  <p className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider leading-relaxed mb-3">
                    Profile edits require direct authorization from the Command Center Administrator.
                  </p>
              ...
              <div className="p-4 bg-amber-950/30 border-l-4 border-amber-500 rounded-r-lg flex items-start gap-3 shadow-lg">
                ...
                <div className="flex-1">
                  ...
                  {!showRequestForm && (
                    ...
                    >
                      <Send size={12} /> {requestStatus === "Denied" ? "Request Again" : "Request Edit Authorization"}
                    </button>
                  )}
                </div>
              </div>
            ...
            {showRequestForm && (
              <form onSubmit={handleSendRequest} className="p-4 bg-zinc-900 border border-zinc-800 rounded-lg animate-in fade-in slide-in-from-top-2 shadow-inner">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  >
                    {isSubmittingRequest ? <Loader2 size={12} className="animate-spin" /> : <Send size={12} />}
                    Submit Request
                  </button>
                </div>
              </form>
        ...
        {errorMsg && (
          <div className="mb-6 p-3 bg-red-950/30 border-l-2 border-red-500 rounded-r-lg flex items-center gap-2">
            <span className="text-xs font-mono text-red-400 uppercase tracking-wider">{errorMsg}</span>
          </div>
        )}
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 space-y-2 mt-4 pt-6 border-t border-zinc-800/50">
              ...
              <p className="text-[10px] font-mono text-zinc-600 mb-2">Leave blank to keep current passcode.</p>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-4 pt-8 border-t border-zinc-800/50 mt-8">
            ...
            >
              {isSaving ? (
                <><Loader2 size={16} className="animate-spin text-zinc-900" /> Updating Data...</>
              ) : saveSuccess ? (
